        ...
        stage('Verify GitHub Auth & Rate Limit') {
            steps {
                script {
                    ...
                    // Your exact original double-quoted string layout that worked earlier
                    def psScript = """
                        \$token = "${env.GITHUB_CREDS_PSW}"
                        \$user  = "${env.GITHUB_CREDS_USR}"
                        
                        # Restored to your exact unquoted format. Removing the inner typo quotes fixes the parser instantly.
                        \$pair   = \${user}:\${token}
